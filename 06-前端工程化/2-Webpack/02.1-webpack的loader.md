...
如果需要给某些 css 加上类`moz`这样的前缀，需要`postcss-loader`以及 autoprefixer 插件。postcss-loader 的作用类似 babel，在其自身一些插件的作用下，可以支持更先进的 CSS 语法。
...
//第二步：webpack.config.js 中的loader配置
            {
                test:  /\.(css|scss)$/,
                use:[
                  'style-loader',
...
使用 file-loader 与 style-loader 配合，此时 import 的 css 不再以 style 标签的形式插入页面，而是生成 link 标签引入打包的 css，import 了多个 css，则生成多个 link，由于该方式会造成多次请求，所以很少使用。
